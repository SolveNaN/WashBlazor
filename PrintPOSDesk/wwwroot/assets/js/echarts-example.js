(function (factory) {
  ...
  const pieChartInit = async () => {
    ...
    if ($chartEl) {
        ...
        try {
            const response = await fetch('https://localhost:7186/api/Graficas');
            const data = await response.json();

            // Mapea los datos para ajustarlos a tu estructura de datos
            const mappedData = data.map(item => ({
                value: item.value,
                name: item.name,
                itemStyle: {
                    color: getColor(item.itemStyle.color)
                }
            }));
